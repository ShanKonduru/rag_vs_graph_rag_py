...
# Try to import document processing functions
try:
...
class GraphVisualizer:
    ...
    def create_sample_graphs(self) -> Dict[str, nx.Graph]:
        ...
        ai_nodes = [
            ...
            (
                "ML",
                {"type": "concept", "description": "Machine Learning", "importance": 9},
            ),
            (
                "DL",
                {"type": "concept", "description": "Deep Learning", "importance": 8},
            ),
            (
                "NLP",
                {
            ...
            (
                "CV",
                {"type": "concept", "description": "Computer Vision", "importance": 7},
            ),
            (
                "RL",
                {
            ...
            (
                ...
                {
                    "type": "organization",
                    "description": "Social Media & AI Company",
                    "importance": 7,
                },
            ),
        ]
        ...
        ai_edges = [
            ...
            (
                "AI",
                "Autonomous_Vehicles",
                {"relationship": "applied_in", "strength": 0.9},
            ),
        ...
        documents = [f"doc_{i}" for i in range(1, 25)]
        ...
        for entity_type, entity_list in entities.items():
            for entity in entity_list:
                entity_graph.add_node(
                    entity, type=entity_type, influence=random.uniform(0.1, 1.0)
                )

        # Add complex relationships
        all_entities = [
            entity for entity_list in entities.values() for entity in entity_list
        ]
    ...
    def load_document_chunks(self) -> List[Dict]:
        ...
        if not chunks_file.exists():
            st.warning(
                "⚠️ No document chunks found. Please run document ingestion first."
            )
            return []
        ...
        try:
            ...
            for chunk_meta in chunks_metadata:
                    ...
                    if source_file.exists():
                        try:
                            ...
                            if file_extension in [".pdf", ".docx", ".pptx"]:
                                # For binary files, use the content from chunk metadata or a placeholder
                                content = chunk_meta.get(
                            ...
                            # Use content from metadata if available, otherwise placeholder
                            content = chunk_meta.get(
                                "content", f"[Could not read: {source_file.name}]"
                            )
                            if (
                                not content
                                or content == f"[Could not read: {source_file.name}]"
                            ):
                                # Don't show warnings for expected binary files
                ...
                chunk_data = {**chunk_meta, "content": content}
    ...
    def extract_entities_from_text(self, text: str) -> List[str]:
        """Enhanced entity extraction from text"""
        ...
        # Find multi-word capitalized phrases (up to 4 words)
        phrases = re.findall(r"\b[A-Z][a-z]+(?:\s+[A-Z][a-z]+){1,3}\b", text)

        # Technical terms and acronyms (2-6 letters)
        tech_terms = re.findall(r"\b[A-Z]{2,6}\b", text)

        # Domain-specific patterns for your knowledge base
        domain_patterns = [
            r"\b[A-Z][a-z]+\s+[A-Z][a-z]+\s+[A-Z][a-z]+\b",  # Three-word proper nouns
            r"\b[A-Z]{2,}\s*\d+(?:\.\d+)?\b",  # Versions like API 2.0, ML 3.5
        ...
        # Filter out common words but be less restrictive
        common_words = {
            "The",
            "This",
            "That",
        ...
        for entity in entities:
            ...
            if entity in common_words:
                continue
            # Keep if it's longer than 2 characters
            if len(entity) <= 2:
                continue
            # Skip if it's all numbers
            if entity.isdigit():
                continue
            # Keep technical terms and meaningful entities
        ...
        # Return more entities but still limit for performance
        return unique_entities[:50]  # Increased from 20 to 50
    ...
    def create_document_relationship_graph(self) -> nx.Graph:
        ...
        for doc_id, doc_chunks in documents.items():
            ...
            doc_name = (
                doc_chunks[0].get("source", doc_id).split("\\")[-1].split("/")[-1]
            )
        ...
        for i, doc1 in enumerate(doc_names):
            for doc2 in doc_names[i + 1 :]:
                ...
                if shared_entities:
                    similarity = len(shared_entities) / len(
                        doc_entities[doc1] | doc_entities[doc2]
                    )
    ...
    def create_entity_relationship_graph(self) -> nx.Graph:
        ...
        # Lower the threshold to show more entities
        significant_entities = {
            entity: docs
            for entity, docs in entity_docs.items()
            if len(entity_chunks[entity]) >= 1
            and len(entity) > 2  # More lenient filtering
        }

        # If we have too many entities, filter to the most significant ones
        if len(significant_entities) > 100:
            # Sort by chunk count and take top 100
            sorted_entities = sorted(
                significant_entities.items(),
                key=lambda x: len(entity_chunks[x[0]]),
                reverse=True,
            )
            significant_entities = dict(sorted_entities[:100])
        ...
        for entity, docs in significant_entities.items():
            ...
            elif entity.lower().endswith(("ing", "tion", "ism", "ity", "ment", "ness")):
                entity_type = "concept"
            elif any(
                word in entity.lower()
                for word in ["strategy", "management", "leadership", "governance"]
            ):
            ...
            entity_graph.add_node(
                ...
                importance=min(
                    chunk_count / 3.0, 1.0
                ),  # More generous importance calculation
                size=min(20 + chunk_count * 5, 100),  # Smaller but more visible nodes
            )

        # Add relationships between entities that co-occur
        entities = list(significant_entities.keys())
        edge_count = 0
        max_edges = 500  # Limit edges for performance
        ...
        for i, entity1 in enumerate(entities):
            ...
            for entity2 in entities[i + 1 :]:
                ...
                if shared_chunks:
                    ...
                    # More generous relationship creation
                    if co_occurrence >= 1:  # Any co-occurrence is significant
                        strength = min(co_occurrence / 2.0, 1.0)  # Lower threshold
    ...
    def create_knowledge_base_graphs(self) -> Dict[str, nx.Graph]:
        ...
        # Create real knowledge graph from your documents
        chunks = self.load_document_chunks()
        if chunks:
            # Build actual knowledge graph
            knowledge_graph = self.create_real_knowledge_graph()
            graphs["Knowledge Graph"] = knowledge_graph
        else:
            # Fallback to sample if no data
            graphs["Knowledge Graph (Sample)"] = self.create_sample_graphs()[
                "AI Knowledge Graph"
            ]

        # Add real data graphs
        graphs["Document Relationship Graph"] = (
            self.create_document_relationship_graph()
        )
        graphs["Entity Relationship Graph"] = self.create_entity_relationship_graph()

        return graphs

    def create_real_knowledge_graph(self) -> nx.Graph:
        """Create a comprehensive knowledge graph from your actual documents"""
        chunks = self.load_document_chunks()

        if not chunks:
            return self.create_sample_graphs()["AI Knowledge Graph"]

        kg = nx.Graph()

        # Extract key concepts from your documents
        document_concepts = defaultdict(set)
        concept_frequency = defaultdict(int)

        # Define technology/concept patterns specific to your domain
        tech_patterns = {
            "AI/ML": [
            ...
            "Security": ["security", "privacy", "compliance", "risk", "cybersecurity"],
        }
                ...
                for category, patterns in tech_patterns.items():
                    for pattern in patterns:
                        if pattern in content:
                            document_concepts[doc_id].add(category)
                            concept_frequency[category] += 1

        # Add concept nodes with importance based on frequency
        for concept, freq in concept_frequency.items():
            importance = min(freq / 10, 1.0)  # Normalize importance
            kg.add_node(
                concept,
                type="concept",
                frequency=freq,
        ...
        # Add document nodes (sample of key documents)
        key_docs = list(document_concepts.keys())[:20]  # Limit for visualization
        for doc_id in key_docs:
            # Clean document name for display
            clean_name = (
                doc_id.replace(".pdf", "").replace(".docx", "").replace(".md", "")
            )
            ...
            kg.add_node(
                clean_name,
                type="document",
                concepts=len(document_concepts[doc_id]),
                size=10 + len(document_concepts[doc_id]) * 2,
            )

            # Connect documents to their concepts
            for concept in document_concepts[doc_id]:
                if concept in kg.nodes():
                    kg.add_edge(
                        clean_name, concept, relationship="contains", strength=0.7
                    )

        # Add concept-to-concept relationships based on co-occurrence
        concepts = list(concept_frequency.keys())
        for i, concept1 in enumerate(concepts):
            for concept2 in concepts[i + 1 :]:
                # Count co-occurrence in documents
                cooccur = sum(
                    1
                    for doc_concepts in document_concepts.values()
                    if concept1 in doc_concepts and concept2 in doc_concepts
                )

                if cooccur >= 2:  # Minimum co-occurrence threshold
                    strength = min(cooccur / 5, 1.0)
                    kg.add_edge(
                        concept1,
                        concept2,
    ...
    def render_header(self):
        ...
        st.markdown(
            '<h1 class="main-header">🕸️ Knowledge Graph & Graph RAG Visualizer</h1>',
            unsafe_allow_html=True,
        )
        ...
        data_source_color = (
            "#28a745" if self.data_source == "knowledge_base" else "#ffc107"
        )
    ...
    def render_sidebar_status(self):
        ...
        if self.data_source == "knowledge_base":
            st.sidebar.markdown(
                """
            - 🧠 **Knowledge Graph**: Key concepts from your 132 documents
            - 📄 **Document Graph**: Relationships between your files
            - 👥 **Entity Graph**: Entities extracted from your content
            - 🔍 **Interactive Analysis**: Explore your knowledge network
            """
            )
        else:
            st.sidebar.markdown(
    ...
    def create_interactive_graph_plot(self, graph: nx.Graph, title: str) -> go.Figure:
        ...
        node_trace = go.Scatter(
            ...
            marker=dict(
                size=[
                    max(8, len(list(graph.neighbors(node))) * 3)
                    for node in graph.nodes()
                ],
                color=[
                    self.get_node_color(graph.nodes[node].get("type", "unknown"))
                    for node in graph.nodes()
                ],
        ...
        for edge in graph.edges():
            ...
            edge_info.append(
                f"{edge[0]} ↔ {edge[1]}: {edge_data.get('relationship', 'connected')}"
            )
            ...
            if len(largest_cc) > 1:
                ...
                try:
                    ...
                    metrics["avg_path_length"] = (
                        f"{nx.average_shortest_path_length(subgraph):.2f} (largest component)"
                    )
                except:
                    pass
    ...
    def render_graph_analysis(self, graph: nx.Graph, graph_name: str):
        ...
        if complexity_score <= 3:
            ...
            st.markdown(
                "🟡 **Moderate Complexity** - Good balance of structure and connections"
            )
        else:
            st.markdown(
                "🔴 **High Complexity** - Dense network with many interconnections"
            )
            ...
            with col2:
                if neighbors:
                    ...
                    for neighbor in neighbors[:10]:  # Show max 10 neighbors
                        ...
                        st.markdown(
                            f"• {neighbor} ({neighbor_type}) - *{relationship}*"
                        )
    ...
    def render_neo4j_integration(self):
        ...
        if not self.neo4j_available:
            st.warning(
                "Neo4j is not available. Please start the Docker services to explore live knowledge graphs."
            )
    ...
    def run(self):
        ...
        with tab1:
            ...
            if self.data_source == "knowledge_base":
                ...
                if chunks:
                    st.markdown(
                        "**🎯 Real Knowledge Graph** built from your 132 documents"
                    )
                    st.markdown(
                        "Showing key concepts, technologies, and relationships from your CTO training materials, case studies, and technical documentation."
                    )

                    # Use actual knowledge graph
                    graph_key = "Knowledge Graph"
                else:
                    st.markdown(
                        "Sample AI concept relationships (demonstration purposes)"
                    )
                    graph_key = "Knowledge Graph (Sample)"
            else:
                st.markdown(
                    "Explore the relationships between AI concepts, technologies, and applications."
                )
                graph_key = "AI Knowledge Graph"

            # Get the appropriate graph
            if graph_key in self.sample_graphs:
                graph = self.sample_graphs[graph_key]

                # Visualization
                fig = self.create_interactive_graph_plot(
                    graph, "Knowledge Graph Network"
                )
                st.plotly_chart(fig, use_container_width=True)

                # Analysis
                self.render_graph_analysis(graph, "Knowledge Graph")

                # Node explorer
                self.render_node_explorer(graph)
            else:
                st.warning("Graph data not available. Please check data loading.")
        ...
        with tab2:
            ...
            if self.data_source == "knowledge_base":
                ...
                if chunks:
                    st.markdown(
                        f"**📊 Knowledge Base Status:** {len(chunks)} document chunks loaded from your data"
                    )
                    documents = len(
                        set([chunk.get("document_id", "") for chunk in chunks])
                    )
                ...
                st.markdown(
                    "Shows relationships between your actual documents based on shared entities and content."
                )
            else:
                st.markdown(
                    "Visualize how documents are connected through shared topics and content similarity."
                )
        ...
        with tab3:
            ...
            if self.data_source == "knowledge_base":
                ...
                if chunks:
                    ...
                    st.markdown(
                        f"**🏷️ Entities Extracted:** {unique_entities} unique entities from your documents"
                    )
                    st.markdown(
                        "**🎯 Real Entity Network** built from your knowledge base showing entity relationships and co-occurrences."
                    )

                    # Use the real entity relationship graph
                    graph = self.sample_graphs["Entity Relationship Graph"]
                else:
                    st.warning(
                        "⚠️ No document chunks found. Please run document ingestion first."
                    )
                    graph = self.create_sample_graphs()["Entity Relationship Graph"]
            else:
                st.markdown(
                    "Complex networks showing relationships between people, organizations, technologies, and concepts."
                )
                graph = self.sample_graphs.get(
                    "Entity Relationship Graph",
                    self.create_sample_graphs()["Entity Relationship Graph"],
                )

            # Check if graph has nodes
            if len(graph.nodes()) == 0:
                st.warning(
                    "⚠️ No entities found to display. The entity extraction may need adjustment for your document content."
                )
                st.info(
                    "This could happen if the entity extraction patterns don't match your document content well."
                )
            else:
                # Display graph statistics
                st.markdown(
                    f"**📊 Graph Stats:** {len(graph.nodes())} entities, {len(graph.edges())} relationships"
                )

                # Visualization
                fig = self.create_interactive_graph_plot(
                    graph, "Entity Relationship Network"
                )
                st.plotly_chart(fig, use_container_width=True)
