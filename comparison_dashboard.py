...
class RAGComparator:
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    try:
                        with knowledge_graph.driver.session(database=config.neo4j.database) as session:
                ...
                if kg_connected and knowledge_graph and 'RAG' in self.retrievers:
                    ...
                    try:
                        ...
                        st.sidebar.warning(f"⚠️ Graph RAG system failed: {str(e)[:50]}...")
                    ...
                    try:
                        ...
                        st.sidebar.warning(f"⚠️ Knowledge Graph system failed: {str(e)[:50]}...")
                else:
                    if not kg_connected:
                        st.sidebar.info("ℹ️ Graph systems will be simulated in demo mode")
    ...
    def query_system(self, system_name: str, query: str) -> Dict[str, Any]:
        ...
        if self.demo_mode or not FULL_SYSTEM_AVAILABLE:
            ...
            if "[Graph RAG Challenge]" in query:
            ...
            return {
                "response": demo_responses.get(system_name, f"Demo response for {query}"),
                "response_time": response_time,
                "retrieved_chunks": np.random.randint(3, 8),
                "confidence_score": confidence,
                "source_diversity": np.random.uniform(0.5, 0.9),
                "graph_nodes_used": np.random.randint(5, 20) if "Graph" in system_name else 0
            }
        ...
        try:
            ...
            llm_request = LLMRequest(
                messages=[{"role": "user", "content": f"Context: {retrieved_context.get('text', '')}\n\nQuestion: {query}\n\nAnswer:"}],
                temperature=0.7,
                max_tokens=512
            )
            ...
            return {
                ...
                "retrieved_chunks": len(retrieved_context.get('chunks', [])),
                "confidence_score": retrieved_context.get('confidence', 0.0),
                "source_diversity": retrieved_context.get('diversity', 0.0),
                "graph_nodes_used": retrieved_context.get('graph_nodes', 0)
            }
    ...
    def compare_systems(self, query: str, selected_systems: List[str]) -> Dict[str, Dict[str, Any]]:
...
def render_header():
    ...
    st.markdown('<h1 class="main-header">🔍 RAG Systems Performance Comparison</h1>', 
                unsafe_allow_html=True)
...
def render_system_selector(comparator: RAGComparator):
    ...
    selected_systems = st.sidebar.multiselect(
        ...
        available_systems,
        default=available_systems[:3] if len(available_systems) >= 3 else available_systems,
...
def render_educational_insights():
    ...
    with st.expander("📚 Why Some Questions Are Challenging for Standard RAG", expanded=False):
        st.markdown("""
    ...
    with st.expander("🎯 Question Types That Showcase Graph RAG", expanded=False):
        ...
        st.info("💡 **Tip**: Try the '[Graph RAG Challenge]' questions in the dropdown to see these advantages in action!")
...
def render_performance_metrics(results: Dict[str, Dict[str, Any]]):
    ...
    for i, (idx, row) in enumerate(df_sorted.iterrows()):
        if i < len(rankings):
            with cols[i]:
                ...
                css_class = css_classes[i] if i < len(css_classes) else "performance-third"
                st.markdown(f"""
                <div class="{css_class}">
                    {ranking}<br>
    ...
    fig = make_subplots(
        rows=2, cols=2,
        subplot_titles=("Response Time", "Retrieved Chunks", "Confidence Score", "Source Diversity"),
    ...
    # Response Time
    fig.add_trace(
        go.Bar(x=systems, y=df["Response Time (s)"], 
    ...
    # Retrieved Chunks
    fig.add_trace(
        go.Bar(x=systems, y=df["Retrieved Chunks"], 
               marker_color=colors, name="Retrieved Chunks"),
        row=1, col=2
    )
    
    # Confidence Score
    fig.add_trace(
        go.Bar(x=systems, y=df["Confidence Score"], 
               marker_color=colors, name="Confidence Score"),
        row=2, col=1
    )
    
    # Source Diversity
    fig.add_trace(
        go.Bar(x=systems, y=df["Source Diversity"], 
               marker_color=colors, name="Source Diversity"),
        row=2, col=2
    )
...
def render_insights_panel(results: Dict[str, Dict[str, Any]]):
    ...
    highest_conf_system = max(results.keys(), key=lambda x: results[x]['confidence_score'])
    highest_conf = results[highest_conf_system]['confidence_score']
    
    # Find most diverse
    most_diverse_system = max(results.keys(), key=lambda x: results[x]['source_diversity'])
    ...
    if "Graph RAG" in results:
        ...
        if "RAG" in results:
            ...
            if time_diff < 20:  # Less than 20% slower
                st.success(f"✅ Graph RAG shows only {time_diff:.1f}% slower response time while providing richer context!")
            else:
                st.info(f"ℹ️ Graph RAG is {time_diff:.1f}% slower but provides enhanced relationship understanding.")
    ...
    for system, result in results.items():
        if "Graph" in system and "Knowledge Graph" not in system:
            trade_offs.append(f"**{system}**: Higher complexity (+{result['response_time']:.1f}s), richer context, better for relationship queries")
        elif "Knowledge Graph" in system:
            trade_offs.append(f"**{system}**: Structured queries, explainable results, best for factual questions ({result['response_time']:.1f}s)")
        else:
            trade_offs.append(f"**{system}**: Fast retrieval ({result['response_time']:.1f}s), good for general questions, lower resource usage")
    ...
    if any("[Graph RAG Challenge]" in key for key in results.keys() if isinstance(key, str)) or "[Graph RAG Challenge]" in str(results):
        st.success("""
        ...
        if "Graph RAG" in results:
            ...
            if graph_rag_conf > 0.8:
                st.success(f"✅ Graph RAG shows high confidence ({graph_rag_conf:.2f}) on this relationship question!")
    
    elif any("[Standard]" in key for key in results.keys() if isinstance(key, str)) or "[Standard]" in str(results):
        st.info("""
    ...
    if best_time < 1.0:
        recommendations.append("⚡ **Speed Priority**: Standard RAG recommended for time-critical applications")
